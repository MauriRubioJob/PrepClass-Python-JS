...
class Wall(arcade.Sprite):
    ...
    def __init__(self, walls, x, y):
        super().__init__()
        self.center_x = Wall.textures[0][0].width * (x + 0.5) * Wall.scale
        self.center_y = Wall.textures[0][0].height * (y + 0.5) * Wall.scale
        

        self.up = walls[y - 1][x] if y > 0 else Wall.edged
        self.ri = walls[y][x + 1] if x < len(walls[y]) - 1 else Wall.edged
        ...
        neighbours = [bool(self.ri), bool(self.do), bool(self.le), bool(self.up)]
        neighbours_sum = sum(neighbours)
        subtexture = 0
        # if x > 11:
        #     print("{},{}:{},{}".format(x, y, neighbours, neighbours_sum))
        if neighbours_sum == 1:
        ...
        self.texture = Wall.textures[neighbours_sum][subtexture]
